...
function buildCalendar() {
  ...
  years.forEach(year => {
    ...
    DOW.forEach((lbl, i) => {
      ...
      el.style.cssText = 'font-family:"DM Mono",monospace;font-size:9px;color:var(--muted);height:12px;line-height:12px;text-align:right;width:18px;';
    ...
    for (let w = 0; w < totalWeeks; w++) {
      ...
      cell.style.cssText = 'width:12px;flex-shrink:0;font-family:"DM Mono",monospace;font-size:9px;color:var(--muted);white-space:nowrap;overflow:visible;';
    ...
    for (let w = 0; w < totalWeeks; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        cell.style.cssText = 'width:12px;height:12px;border-radius:2px;flex-shrink:0;';
...
function updatePnlPreview() {
  ...
  const pnl = sell - buy - Math.abs(tax);
...
async function saveTrade() {
  ...
  const pnl = parseFloat((sell - buy - Math.abs(tax)).toFixed(2));
...
async function saveEdit() {
  ...
  const pnl = +(sell - buy - Math.abs(tax)).toFixed(2);
...
function parseXlsx(buffer) {
  ...
  filtered.forEach(row => {
    const isin = row.isin;
    if ((isinBuyCost[isin] || 0) < KNOCKOUT_THRESHOLD) return;
    ...
    if (row.type === 'Buy' && shares > 0) {
      ...
      const pnl = +(sellRev - cost - Math.abs(tax)).toFixed(2);
      const uid = isin + '_' + dateStr + '_' + sellRev.toFixed(2) + '_' + shares.toFixed(3);
      closed.push({ uid, date: dateStr, isin, desc: row.description, broker: 'scalable', shares, buy: +cost.toFixed(2), sell: +sellRev.toFixed(2), tax: +Math.abs(tax).toFixed(2), pnl });
    }
  });

  const newOpenLots = [];
  Object.values(buyPools).forEach(pool => pool.forEach(lot => { if (lot.shares > 0.001) newOpenLots.push(lot); }));
